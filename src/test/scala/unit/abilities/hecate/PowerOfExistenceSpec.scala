    ...
    with TestUtils
{
  ...
  PowerOfExistence.metadata.name must {
    "be able to damage characters" in {
      ...
      val r = validator.validateAbilityUseOnCoordinates(p0FirstCharacter.owner.id, abilityId, p1FirstCharacterSpawnCoordinates)
      assertCommandSuccess(r)

      val abilityUsedGameState: GameState = gameState.useAbilityOnCoordinates(abilityId, p1FirstCharacterSpawnCoordinates)
