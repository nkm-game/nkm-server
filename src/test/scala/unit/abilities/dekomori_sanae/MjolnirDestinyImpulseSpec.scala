    ...
    with TestUtils
{
  ...
  private val aGsNoRefresh0: GameState =
    s.ultGs
      .useAbilityOnCoordinates(s.defaultAbilityId, s.p(0)(1).spawnCoordinates)
    ...
    "not be able to use again if no enemies were killed" in {
      assertCommandFailure {
        GameStateValidator()(aGsNoRefresh0)
          .validateAbilityUseOnCoordinates(s.owners(0), s.defaultAbilityId, s.p(0)(1).spawnCoordinates)
      }
      assertCommandFailure {
