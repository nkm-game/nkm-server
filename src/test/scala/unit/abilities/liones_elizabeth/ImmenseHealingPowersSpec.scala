    ...
    with TestUtils
{
  ...
  private val potgAbilityId =
    s.characters.p0First.state.abilities(3).id

  private def dmgGs(dmg: Int): GameState =
    s.gameState.damageCharacter(s.characters.p0Second.id, Damage(DamageType.True, dmg))
  ...
  private def invigorateAppliedGs(gs: GameState): GameState =
    gs.useAbilityOnCharacter(invigorateAbilityId, s.characters.p0Second.id)
      .endTurn()
  ...
  abilityMetadata.name must {
    ...
    "heal stronger based on missing HP (potg)" in {
        val heal1 = potgUsedGs(dmgGs10).gameLog.events.ofType[CharacterHealed].find(_.characterId == s.characters.p0Second.id).head.amount
        val heal2 = potgUsedGs(dmgGs60).gameLog.events.ofType[CharacterHealed].find(_.characterId == s.characters.p0Second.id).head.amount
        val heal3 = potgUsedGs(dmgGs95).gameLog.events.ofType[CharacterHealed].find(_.characterId == s.characters.p0Second.id).head.amount
    ...
    "not heal stronger when healed by system" in {
      val heal1 = dmgGs10.heal(s.characters.p0Second.id, 10).gameLog.events.ofType[CharacterHealed].head.amount
      val heal2 = dmgGs60.heal(s.characters.p0Second.id, 10).gameLog.events.ofType[CharacterHealed].head.amount
      val heal3 = dmgGs95.heal(s.characters.p0Second.id, 10).gameLog.events.ofType[CharacterHealed].head.amount
